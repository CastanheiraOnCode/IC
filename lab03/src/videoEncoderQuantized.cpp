...
int quantize(int value , int quantizationLevel){
    
    value = value - (value % quantizationLevel);
    ...
    for (int k=0;k<3;k++){
        ...
        for (i = 0; i < height; i++)
        {
            ...
            for (j = 0; j < width; j++)
            {
                ...
                encodedResidual += g.encode(quantize(R, quantizationLevel));
                numOfValues++;
                }
    ...
    for (int i = 0; i < block.rows; i++)
    {
        for (int j = 0; j < block.cols; j++)
        {
            if (block.at<uchar>(i, j) != bestMatch.at<uchar>(i, j))
            {
                ...
                string encodedDiff = g.encode(j)+ g.encode(i) + g.encode(quantize(diffB,quantizationLevel)) + g.encode(quantize(diffG,quantizationLevel)) + g.encode(quantize(diffR,quantizationLevel));
...
//Main argument function receives <video file> <search window> <block size> <period of key frames>  <quantization factor>

int main(int argc, char** argv){

    FrameNumber=0;
    fp = fopen("out.txt", "a");
    quantizationLevel = atoi(argv[5]);
